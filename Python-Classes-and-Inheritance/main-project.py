...
class WOFPlayer:
    ...
    def addMoney( self, amt):
        self.prizeMoney += amt
    #When the wheel lands on bankrupt.
    def goBankrupt(self):
        self.prizeMoney = 0
    #Adding the prize
...
class WOFHumanPlayer(WOFPlayer):
    ...
    def getMove(self,category, obscuredPhrase, guessed):
        ...
        player_choice = str(input("Guess a letter, phrase, or type 'exit' or 'pass': "))
        return player_choice

#Computer player's sub-class.
        ...
        if Guess == []:
            ...
            if the_value == True:
                ...
                while(0 <= i <= len(self.SORTED_FREQUENCIES)-1):
                    if self.SORTED_FREQUENCIES[i] in Guess:
                        return self.SORTED_FREQUENCIES[i]
                    else:
                        i-=1
                        continue
            else:
                rand_letter = random.choice(Guess)
                return rand_letter
